...
% Rule to return what is available at each taqueria.
available_at(X,Y) :- taqueria(Y,_,Z),isin(X,Z).

% Checks if item x is available in more than one taqueria
multi_available(X) :- bagof(Y,available_at(X,Y),Z),Z=[_|T],not(T=[]).

% Helper function for overworked
overworked_helper(X,Y) :- taqueria(Y,Z,_),isin(X,Z).
% This rule which checks if X works at more than one taqueria
overworked(X) :- bagof(Y,overworked_helper(X,Y),Z),Z=[_|T],not(T=[]).

% Helper function for total_cost which adds up the total cost for the ingredients.
adder([],0).
adder(Z,S) :- Z=[H|T],cost(H,P),adder(T,R),S is P+R.
% Checks if the sum of the costs of the ingredients of items X is equal to K
total_cost(X,K) :- ingredients(X,Y),adder(Y,K).

% Checks if L is a list of the ingredients to item X.
has_ingredients(X,L) :- ingredients(X,Y),union(Y,L,Y).

% This predicate checks if L does not have any ingredients in item X.
avoids_ingredients(X,L) :- ingredients(X,Y),intersection(Y,L,Z),(Z=[]).

% BOTH p1 and p2 are helper functions for the find_items functions.
p1(L,X) :- bagof(Z,has_ingredients(Z,X),L).

p2(L,Y) :- bagof(Z,avoids_ingredients(Z,Y),L).

% This function checks if L is the list of all items that contain all the ingredients in x and not in Y.
