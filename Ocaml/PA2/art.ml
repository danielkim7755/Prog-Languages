 ...
 *
 * This function is identical to the build function except that it
 * includes the new operators
 *)
         ...
         | _ -> 
            let y = rand(1,10) in
               match y with
                  ...
                  | 5 -> buildTimes(build (rand,depth-1), build (rand,depth-1))
                  | 6 -> buildThresh(build (rand,depth-1), build (rand,depth-1), build (rand,depth-1), build (rand,depth-1))
                  | 7 -> buildInversePos (build (rand,depth-1))
                  | 8 -> buildInverseNeg (build (rand,depth-1))
                  | 9 -> buildProductRootPos (build (rand, depth-1), build (rand,depth-1), build (rand,depth-1))
                  | 10 -> buildProductRootNeg (build (rand, depth-1), build (rand,depth-1), build (rand,depth-1))
                  | _ -> build (rand,depth)
